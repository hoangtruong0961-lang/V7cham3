...
export const ensureThinkingTagsBalanced = (text: string): string => {
  ...
  temp = temp.replace(/<\/(thinking|think|thinhking|thought|thoughts)\b[^>]*>/gi, '</thinking>');
  temp = temp.replace(/<\s*\/+\s*(thinking|think|thinhking|thought|thoughts)\s*>/gi, '</thinking>');
  temp = temp.replace(/\\(thinking|think|thinhking|thought|thoughts)>/gi, '</thinking>');
  ...
  const contentOpenRegex = /<(content|story|branches|choices)\b[^>]*>/i;
  ...
  tagsToRemove.forEach(tag => {
    ...
    while (true) {
        ...
        if (closingIndex !== -1) {
            ...
            if (['thinking', 'think', 'thinhking', 'thought', 'thoughts'].includes(tag)) {
                ...
                for (let j = 0; j < remainingLines.length; j++) {
                    ...
                    if (!l) {
                        accumulatedChars += remainingLines[j].length + 1;
                        continue;
                    }
                    
                    const isDialog = l.match(/^[“"「『—-]\s*[A-ZÀ-Ỹ]/) || l.startsWith('“') || l.startsWith('「') || l.startsWith('|') || l.startsWith('『');
                    const isNarrativeProse = l.length > 50 && 
                        !l.startsWith('-') && 
                        !l.startsWith('*') && 
                        !l.startsWith('•') && 
                        !l.startsWith('[') && 
                    ...
                    if (accumulatedChars > 100 && (isDialog || isNarrativeProse)) {
                        splitLineIdx = j;
                        break;
                    }
                    accumulatedChars += remainingLines[j].length + 1;
